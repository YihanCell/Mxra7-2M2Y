...
cell_type_estimateDisp <- function(cell_type){
  ...
  saveRDS(dge.subset, paste0("./Result_for_WT/05.edgeRMarkgenes.Result/", gsub(" ", "_", cell_type), "_Disp.rds"))
  
  print("Building model...")
  fit = glmFit(dge.subset, design, robust = TRUE)
  lrt = topTags(glmLRT(fit), n = nrow(dge.list$counts))
  write.table(lrt, paste0("./Result_for_WT/05.edgeRMarkgenes.Result/",gsub(" ", "_", cell_type),"control_treat.glmLRT.txt"), sep = '\t', col.names = NA, quote = FALSE)
  gene_diff = read.delim(paste0("./Result_for_WT/05.edgeRMarkgenes.Result/",gsub(" ", "_", cell_type),"control_treat.glmLRT.txt"), row.names = 1, sep = '\t', check.names = FALSE)
  ...
  write.table(gene_diff_select, file = paste0("./Result_for_WT/05.edgeRMarkgenes.Result/", gsub(" ", "_", cell_type), ".control_treat.glmLRT.select.txt"), sep = '\t', col.names = NA, quote = FALSE)
  
  #根据 up 和 down 分开输出
  gene_diff_up <- subset(gene_diff, sig == 'up')
  gene_diff_down <- subset(gene_diff, sig == 'down')
  
  write.table(gene_diff_up, file = paste0("./Result_for_WT/05.edgeRMarkgenes.Result/", gsub(" ", "_", cell_type), ".control_treat.glmLRT.up.txt"), sep = '\t', col.names = NA, quote = FALSE)
  write.table(gene_diff_down, file = paste0("./Result_for_WT/05.edgeRMarkgenes.Result/", gsub(" ", "_", cell_type), ".control_treat.glmLRT.down.txt"), sep = '\t', col.names = NA, quote = FALSE)}
