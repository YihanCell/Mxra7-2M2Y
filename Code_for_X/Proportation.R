...
sub = table(combinedh.X@meta.data$split, combinedh.X@meta.data$predicted_celltype)
sub
prop = prop.table(sub, margin = 1)
prop
cell_count = as.data.frame(table(combinedh.X@meta.data$split, combinedh.X@meta.data$predicted_celltype))
cell_count = as.data.frame(prop)
head(cell_count)
names(cell_count) = c("Group", "CellType", "Percentage")
write.csv(x = cell_count,file = "./Result_for_X/09.Proportation/cell_count.csv")
...
Per1
ggsave('./Result_for_X/09.Proportation/X_CelltypePercentage_nolegend.pdf', plot = Per1, height = 5, width = 5)
...
Per2
ggsave('./Result_for_X/09.Proportation/X_CelltypePercentage.pdf', plot = Per2, height = 5, width = 8)
...
Per3 = ggplot(cell_count, aes(x=CellType, y=Group, fill=Percentage)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=16),
        ...
        axis.title.y = element_text(size=14))
Per3
ggsave('./Result_for_X/09.Proportation/X_CelltypePercentage_Compare.pdf', plot = Per3, height = 5, width = 6)
