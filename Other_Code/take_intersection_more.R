...
SingleR_Subset <- function(cell_type){
  markersX = read.delim(paste0("./Result_for_X/06/", cell_type, ".control_treat.glmLRT.plot.txt"), row.names = 1, sep = '\t', check.names = FALSE)
  gene_upX = markers[which(markersX$sig=='up'),]
  gene_up_nameX = rownames(gene_upX)
  markersWT = read.delim(paste0("./Result_for_WT/05/", cell_type, ".control_treat.glmLRT.plot.txt"), row.names = 1, sep = '\t', check.names = FALSE)
  ...
  writeLines(intersectionUP, paste0("./Result_for_interstection/more/", gsub(" ", "_", cell_type), ".up.txt"))
  ...
  writeLines(intersectionDOWN, paste0("./Result_for_interstection/more/", gsub(" ", "_", cell_type), ".down.txt"))}
