...
goenrichment <- function(cell_type){
  ...
  markers <- read.delim(paste0("./05.edgeRMarkgenes.Result/", cell_type, ".control_treat.glmLRT.plot.txt"), row.names = 1, sep = '\t', check.names = FALSE)
  ...
  ggsave(paste0("./05.edgeRMarkgenes.Result/Enrich_Go/", cell_type, ".up_gene.Go_BP.pdf"), plot = BP.Go.up)
  ...
  ggsave(paste0("./05.edgeRMarkgenes.Result/Enrich_Go/", cell_type, ".up_gene.Go.pdf"), plot = p.Go.up)
  ...
  ego_MF = enrichGO(gene = geneset,
                    ...
                    qvalueCutoff = 0.05,
                    readable = TRUE)
  BP.Go.down = dotplot(ego_BP)
  ggsave(paste0("./05.edgeRMarkgenes.Result/Enrich_Go/", cell_type, ".down_gene.Go_BP.pdf"), plot = BP.Go.down)
  ...
  go_enrich_df$type_order=factor(rev(as.integer(rownames(go_enrich_df))),labels=rev(go_enrich_df$Description))
  COLS = c("#66C3A5", "#8DA1CB", "#FD8D62")
  p.Go.down = ggplot(data=go_enrich_df, aes(x=type_order,y=GeneNumber, fill=type)) + 
    geom_bar(stat="identity", width=0.8) + 
    scale_fill_manual(values = COLS) + 
    coord_flip() + 
    xlab("GO term") + 
    ...
    labs(title = paste0("The Most Enriched GO Terms ",markers.screen_type))+
    theme_bw() + theme(axis.text.y=element_text(size=8, angle=45, hjust=1))
  ggsave(paste0("./05.edgeRMarkgenes.Result/Enrich_Go/", cell_type, ".down_gene.Go.pdf"), plot = p.Go.down)}
